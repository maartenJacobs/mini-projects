...
spec = do
  describe "cutFields" $ do
    it "returns the unchanged row when it does not contain the delimiter" $ do
        cutFields ',' [] "row" `shouldBe` (Left $ ContainsNoDelimiter "row")
    it "returns the unchanged row when all fields are selected" $ do
        -- TODO: generate every ordering of the field numbers and every distribution of every ordering.
        sequence_ [cutFields ',' fs "f1,f2,f3" `shouldBe` Right "f1,f2,f3" | fs <- [[[1..3]], [[1..2], [3]]]]
    it "returns fields in order" $ do
        cutFields ',' [[3,2,1]] "f1,f2,f3" `shouldBe` Right "f1,f2,f3"
    it "returns fields only once" $ do
